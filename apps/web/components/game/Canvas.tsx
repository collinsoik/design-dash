...
export default function Canvas() {
  ...
  const placeholderSections: SectionSlotType[] = useMemo(
    () => [
      ...
      { id: "features", label: "Features Section", assignedTo: null, turnOrder: 3, placedComponent: null, status: "empty" },
      { id: "content", label: "Content Section", assignedTo: null, turnOrder: 4, placedComponent: null, status: "empty" },
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-game-blue/30">
        ...
        {room?.gameState?.caseStudy && (
          <span className="font-pixel text-[7px] text-game-yellow truncate max-w-[200px]">
            {room.gameState.caseStudy.businessName}
          </span>
        )}
        <span className="font-pixel text-[7px] text-gray-600">
          {displaySections.filter((s) => s.status === "placed").length}/
          {displaySections.length} PLACED
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-game-dark/40 p-4">
        ...
        {/* Non-active player overlay */}
        {!isMyTurn && activePlayerId && (
          <div className="fixed inset-0 pointer-events-none z-10 flex items-center justify-center">
            {/* This overlay only blocks canvas interaction; palette is still browsable */}
          </div>
        )}
      </div>
    </div>
